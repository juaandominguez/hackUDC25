...
const SurpriseCard = ({ item }: { item: ClothingItem }) => {
  const [image, setImage] = useState<string | null>(item.image || null)

  const handleView = async () => {
    try {
      const scrapeResponse = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/scrape?url=${encodeURIComponent(item.link)}`
      );

      if (!scrapeResponse.ok) {
        throw new Error(`Failed to scrape ${item.link}`);
      }
  ...
  return (
    <Card className="w-64 h-[400px] shadow-lg overflow-hidden flex flex-col border border-black">
      ...
      <CardContent className="p-4 flex-grow">
        <p className="uppercase font-semibold truncate text-sm whitespace-normal">
          {item.name}
        </p>
      ...
      <CardFooter className="p-4 flex justify-between gap-2">
        ...
        <button
          onClick={handleView}
          className="px-4 py-2 bg-yellow-500 text-black text-sm font-medium uppercase border border-black hover:bg-yellow-400 transition"
        >
          View
        </button>
        <a
