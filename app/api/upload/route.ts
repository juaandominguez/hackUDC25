...
export async function GET(req: any) {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_URL}/api/vision?imageUrl=${pageUrl}`
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/vision?imageUrl=${uploadResult.Location}`
      );
      ...
      const productLinks = data.map(
        (product: { link: string }) => product.link
      );
